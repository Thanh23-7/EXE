...
// THÊM: Import crypto để dùng cho VNPay
...
// THÊM: Định nghĩa URL cơ sở từ biến môi trường (BACKEND_URL đã đặt trên Render)
const BACKEND_BASE_URL = process.env.BACKEND_URL || `http://localhost:${PORT}`;

// Serve static files (HTML, CSS, JS)
// LƯU Ý: Render không dùng static file như cách này, nhưng vẫn giữ lại cho testing.
...
// ... (các lệnh tạo bảng và tài khoản admin giữ nguyên) ...
...
// ... (Các route OAuth và initializeDB giữ nguyên) ...
...
                    db.run('INSERT INTO products (name, price, description, category, image) VALUES (?, ?, ?, ?, ?)', [p.name, p.price, p.description, p.category, p.image]);
                });
            }
        });
    });
